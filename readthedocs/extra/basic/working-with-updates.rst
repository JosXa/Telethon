    ...
    .. code-block:: python
        ...
        def admin_handler(event):
            if event.raw_text.startswith('eval '):
                expression = event.raw_text.replace('eval ', '', 1)
...
random number, whereas if you say ``'eval 4+4'``, your own account will evaluate
the expression as Python code and reply with the solution. Try it!
